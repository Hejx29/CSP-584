...
public class PhoneList extends HttpServlet {
	...
	/* Laptop Page Displays all the Phones and their Information in Game Speed */
	...
	/* Checks the Phones type whether it is microsft or apple or samsung */
		...
		else 
		{
			...
			else if (CategoryName.equals("pixel"))
			{
				for(Map.Entry<String,Phone> entry : SaxParserDataStore.phones.entrySet())
				{
				  if(entry.getValue().getRetailer().equals("Pixel"))
				  {
					 hm.put(entry.getValue().getId(),entry.getValue());
				  }
				}
			...
			else if (CategoryName.equals("honor"))
			{
				for(Map.Entry<String,Phone> entry : SaxParserDataStore.phones.entrySet())
				{
